...
`CM_NODE_ID` 仍然支持显式指定，但已经不再是 Docker 场景下保持节点身份稳定的唯一方式。

当前版本的默认行为：

- Agent 首次生成新的节点标识时，会生成随机 `UUID`，并默认持久化到 `~/.cybermonitor-node-id`
- 裸机 / 常规安装场景下，后续升级会继续复用这个 home 目录下的节点文件
- Docker 场景下，如果你显式设置了 `CM_NODE_ID`，会优先使用显式值
- Docker 场景下，如果没有显式指定 `CM_NODE_ID`，但挂载了 Agent 运行用户的 home（默认 `cm` 用户，对应 `/home/cm`），则会继续复用 `/home/cm/.cybermonitor-node-id`
- Docker 场景下，如果既没有显式 `CM_NODE_ID`，也没有可复用的 home 节点文件，Agent 会退回到宿主机指纹生成的稳定 `UUID`，这样 watchtower 或服务端触发的容器重建后，仍会保持同一个节点身份
...
推荐直接公开 `CM_PUBLIC_LISTEN` 对应的展示页端口。当前版本的公共数据接口会绑定展示页 bootstrap 访问链路，因此如果你要继续做纯静态 CDN 托管，请确保浏览器仍然通过同源代理访问后端 public 入口，而不是直接裸连数据接口。

说明：

- public 数据接口的访问控制是“最小暴露面”设计，不是面向恶意用户的强认证；**任何最终要展示给浏览器的数据，理论上都无法对真正拿到页面的人完全隐藏**
- 因此更稳妥的部署方式仍然是：只公开你愿意展示给访客看到的节点与字段，不要把敏感节点直接挂到公共展示页
...
- `snapshot` 仍然存在，但它现在只负责前台首屏 hydration 与 WebSocket 静默补帧，不再承载完整网络连通性历史
